...
{
	static uint8_t outfig;
	if(TIM2->SR&TIM_IT_Update)
	{					
		if(startDelay >0)
		{	
		//	if(!GPIO_ReadInputDataBit(GPIOA, 6))
		//	{
				TR_ON;	
				startDelay --;
				if(startDelay == 0) SYS.DataGet[Rate] = 0;	
				trun = 7;	//0.8ms		
		//	}else TR_OFF;				
		}		
		else
...
void GetTem(s32 *data)
{
	...
	SYS.DataGet[Tem_PV]=(s16)(PvOut+0.8);
	
  //  SYS.DataGet[Tem_PV]= SYS.DataGet[Tem_sensor
...
void GetCurrent(void)
{
    ...
    if(SYS.DataGet[Current]>1800) SYS.DataGet[Current] = 1800;
	SYS.AD_AddValue[2]=0;
	SYS.AD_num2=0;
}
void Task1(void) //10´Î
{
	...
	if(CountStep<100) 
	{
		...
		{	
			GetCurrent();
			/*if(SYS.DataGet[Current]>30) {SYS.ErrorCode=HT_ST;CountStep=32;SYS.DataGet[Rate]=0;} 
		...
		else if(CountStep==32) GetCurrent();//{GetCurrent();if((SYS.DataGet[Current]==0)&&(GPIOB->IDR & GPIO_Pin_1)&&(GPIOB->IDR & GPIO_Pin_2)) SYS.ErrorCode=HT_OP;}
	...
	else if(CountStep>=100)
	{
		...
		if(SYS.RunningMode<2)
		{
				...
				if(SYS.AutoFlag==4)
			 	{
					...
					else if(i==4)
					{
						...
						ek = SYS.TemSV-SYS.TemPV;
						
						out = MVresult1 + (ek-lastek)*SYS.DataSet[P]*8.0;
												
						SYS.DataGet[Rate]=out/100;	
		    ...
		    StandbyCount=SYS.StandbyTime*5;
            if(SYS.DataGet[Tem_PV]>5000) {  //TC open
                SYS.DataGet[Tem_PV] = 0;
                if(SYS.ErrorCode<3) SYS.ErrorCode=TC_OP;
            }
		}
		...
		{	
			...
			TurnData.step=0;
			SYS.AutoFlag=0;
            if(SYS.DataGet[Tem_PV]>5000) SYS.DataGet[Tem_PV] = 0;
		}
	  ...
	  if(i==5) i=0;
	  else {i++;return;} //50´Î=20*50=1000ms=1s
	  ...
	  if(SYS.DataSet[HSCI]) SYS.SSR_Count=0;
		...
		//****************************************************************Check TC_ST
		if((SYS.TemPID-SYS.DataGet[Tem_E])<100)
		{  //950
			if((SYS.DataGet[Rate]>=300)&&(SYS.ErrorCode!=HT_OP)&&(SYS.ErrorCode!=F1_OP)&&(SYS.ErrorCode!=F2_OP))
			{
				SYS.ErrorCode=TC_ST;
...
//				}
		...
		if((outBak>=200)&&(SYS.DataGet[Current]<5)&&(SYS.ErrorCode!=F1_OP)&&(SYS.ErrorCode!=F2_OP))
		{		
		...
		if((outBak==0)&&(SYS.DataGet[Current]>=20)) {if(++TRST_count==3) SYS.ErrorCode=TR_ST;SYS.ManualRate=0;}//Check TRAC is short?
		else TRST_count=0;

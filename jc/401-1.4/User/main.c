...
void SysTickProc(void) //0.1ms
{
   ...
   if(comRx >0) { comRx --; if(comRx == 0) Rxcom();}
	...
	}else {key=0;turOff=0;} //check,checkOff
}
void DisError(u8 num)
{
	switch(num)
	{
		...
		case HC_RE :	SegLed[0]=ascii['H'];SegLed[1]=ascii['C']|0x8000;SegLed[2]=ascii['R'];SegLed[3]=ascii['E'];break;
		case TEST  :	SegLed[0]=ascii['C'];SegLed[1]=ascii['H']|0x8000;SegLed[2]=ascii['K'];SegLed[3]=ascii['-'];break;
...
int main(void)     
{
	...
	Versions[2] = '1';
	Versions[3] = '4';
	LED[0] = ascii[Versions[0]];
	LED[1] = ascii[Versions[1]];
	LED[2] = ascii[Versions[2]]|0x8000;
	LED[3] = ascii[Versions[3]];	
	LED[4] = ascii['_'];
	LED[5] = ascii['_'];
	LED[6] = ascii['O'];
	if(checkOff) LED[7] = ascii['N']; else LED[7] = ascii['F'];	

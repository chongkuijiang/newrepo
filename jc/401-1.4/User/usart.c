...
//	
    ...
    USART_ClearFlag(USART1,USART_FLAG_TC|USART_IT_RXNE|USART_FLAG_TXE);
...
void USART1_IRQHandler(void)                	//串口1中断服务程序
{
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  
	{
		...
		if(Rxbuf1[0]<10)Rxbuf1[0]++;
		Rxbuf1[Rxbuf1[0]] =USART_ReceiveData(USART1);
		comRx = 3; //0.5ms
    } 
	if (USART_GetITStatus(USART1, USART_IT_TXE) != RESET)
	{		
		    USART_SendData(USART1, Txbuf[p++]);
		   if(Txbuf[1]==0x5a){
				if(p ==17){
...
void uart_init(void)
{
    ...
    uart3_init();
#endif	
//	SYS.DataSet[ID] = comAddrss = 4;//getID();
}

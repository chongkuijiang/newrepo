...
INT UpDownUnsignedValue(INT LowLimit,INT HighLimit,INT dataValue,UCHAR PointEffer,BOOL IncOrDec)
{
	UINT i=1;
...
void ShiftKeyUnlashResponse(UCHAR num)
{
	if((num>2)&&(num<40))
	{
		...
		switch((UINT)SYS.SetMenu*100+SYS.SetStep)
		{
			...
			case 207: case 208:	case 108: case 109:
					if(SYS.BlinkState==0x80) SYS.BlinkState=0x40;
					else if(SYS.BlinkState==0x40) SYS.BlinkState=0x20;
  	...
  	if(num==2) 
  	{
  	  	...
  	  	switch((UINT)SYS.SetMenu*100+SYS.SetStep)
		{
			case 0:
					...
					SYS.DataSet[Tem_SV]=dataBuff+SYS.DataSet[OFS]*10;
			...
			case 108:if(SYS.DataSet[LOC]) {SYS.LockCount=LOCKTIME;break;}
					SYS.DataSet[OFS]=UpDownUnsignedValue(-100,100,SYS.DataSet[OFS],0,1);
					if((SYS.DataSet[OFS]>-100)||(SYS.DataSet[OFS]<100)) SYS.WriteState=OFS;
					else SYS.WriteState=0;
					break;
			case 109:if(SYS.DataSet[LOC]) {SYS.LockCount=LOCKTIME;break;}
			...
			case 203:if(SYS.DataSet[LOC]) {SYS.LockCount=LOCKTIME;break;}
					SYS.DataSet[HC_H]=UpDownUnsignedValue(160,300,SYS.DataSet[HC_H],0,1);
					SYS.WriteState=HC_H;
					break;
			...
			case 209:
					...
					SYS.BlinkState=0x00;
					break;
//			case 210:if(SYS.DataSet[LOC]) {SYS.LockCount=LOCKTIME;break;}
//					SYS.DataSet[P]=UpDownUnsignedValue(1,999,SYS.DataSet[P],0,1);
//					SYS.WriteState=P;
//					break;
			case 300:
					SYS.SetMenu=4;
	...
	if(num==2)
	{
		...
		switch((UINT)SYS.SetMenu*100+SYS.SetStep)
		{
			...
			case 107:
					...
					break;
			case 108:if(SYS.DataSet[LOC]) {SYS.LockCount=LOCKTIME;break;}
					SYS.DataSet[OFS]=UpDownUnsignedValue(-100,100,SYS.DataSet[OFS],0,0);
					if((SYS.DataSet[OFS]>-100)||(SYS.DataSet[OFS]<100)) SYS.WriteState=OFS;
					else SYS.WriteState=0;
					break;
			case 109:if(SYS.DataSet[LOC]) {SYS.LockCount=LOCKTIME;break;}
			...
			case 202:if(SYS.DataSet[LOC]) {SYS.LockCount=LOCKTIME;break;}
					...
					break;
			case 203:if(SYS.DataSet[LOC]) {SYS.LockCount=LOCKTIME;break;}
					SYS.DataSet[HC_H]=UpDownUnsignedValue(160,300,SYS.DataSet[HC_H],0,0);
					SYS.WriteState=HC_H;
					break;
			case 204:if(SYS.DataSet[LOC]) {SYS.LockCount=LOCKTIME;break;}
			...
			case 209:
					...
					break;
//		  case 210:
//               if(SYS.DataSet[LOC]) {SYS.LockCount=LOCKTIME;break;}
//					SYS.DataSet[P]=UpDownUnsignedValue(1,999,SYS.DataSet[P],0,0);
//					SYS.WriteState=P;
//					break;
			case 300:
					SYS.SetMenu=2;

...
BOOL disUpFlag=0;
void ConverToChar(INT data,UINT *resultAddress,UCHAR pointNum,UCHAR PointEffer)
{
	int dat = abs(data);
	UCHAR ps[4]={0,0,0,0};	
	if(dat>=10000) dat=9999;
	if(PointEffer&&!SYS.DataSet[DPSW]) data/=10;
	ps[0]=(UCHAR)(dat/1000%10)+0x30;
	ps[1]=(UCHAR)(dat/100%10)+0x30;
	ps[2]=(UCHAR)(dat/10%10)+0x30;
	ps[3]=(UCHAR)(dat%10)+0x30;
	...
	if(data < 0) resultAddress[0] = ascii['-'];
}
////////////////////////////////////////////////////////////////////////////////////////////
...
{	
	...
	{
		case 0: 
				ConverToCharPV(abs(dataBuff-SYS.DataSet[OFS]*10),&SegLed[0],SYS.DataSet[DPSW],1);
		...
		case 108:
				...
				SegLed[2]=ascii['F'];
				SegLed[3]=ascii['S'];
				ConverToChar(SYS.DataSet[OFS],&SegLed[4],0,0);
				break;
		case 109:
				SegLed[0]=ascii[' '];
				SegLed[1]=ascii[' '];
		...
		case 209:
				...
				break;
		case 210:
				ConverToChar(power,&SegLed[0],0,0);
				ConverToChar(SYS.DataSet[P],&SegLed[4],0,0);
				break;
		case 300:
				SegLed[0]=ascii['_'];
				SegLed[1]=ascii['S'];
...
void LoadData(void)
{
	...
	{
		if(SYS.DataSet[EE]==0xab) CommIIC(SYS.DataSet,100,0,1);
		else DataInit();
	}

...
// 	GPIO_Init(GPIOB,&portSet);
  //零点 保险丝
  portSet.GPIO_Pin=GPIO_Pin_0|GPIO_Pin_1;
	portSet.GPIO_Mode=GPIO_Mode_IN_FLOATING;
	GPIO_Init(GPIOB,&portSet);
	portSet.GPIO_Pin=GPIO_Pin_9|GPIO_Pin_8|GPIO_Pin_7 ;
	portSet.GPIO_Mode=GPIO_Mode_IN_FLOATING;
	GPIO_Init(GPIOE,&portSet);
	portSet.GPIO_Pin=GPIO_Pin_10|GPIO_Pin_11|GPIO_Pin_12|GPIO_Pin_13 ;
	portSet.GPIO_Mode=GPIO_Mode_IN_FLOATING;
	GPIO_Init(GPIOE,&portSet);
//////check	
	portSet.GPIO_Pin=GPIO_Pin_2|GPIO_Pin_3|GPIO_Pin_4|GPIO_Pin_5;
...
// 	GPIO_Init(GPIOB,&portSet);
  //LED
  portSet.GPIO_Pin=GPIO_Pin_14|GPIO_Pin_15;
	portSet.GPIO_Mode=GPIO_Mode_Out_PP;
	GPIO_Init(GPIOB,&portSet);
	portSet.GPIO_Pin=GPIO_Pin_8 ;
	portSet.GPIO_Mode=GPIO_Mode_Out_PP;
...
// 	GPIO_Init(GPIOB,&portSet);
	//待定
// //   portSet.GPIO_Pin= GPIO_Pin_8|GPIO_Pin_9 ;
// //   portSet.GPIO_Mode=GPIO_Mode_IPD;
// // 	GPIO_Init(GPIOB,&portSet);
// 	//环境温度
//   portSet.GPIO_Pin=GPIO_Pin_0;
...
// 	GPIO_Init(GPIOC,&portSet);
	//拨码开关
  portSet.GPIO_Pin=GPIO_Pin_15|GPIO_Pin_14|GPIO_Pin_13|GPIO_Pin_12|GPIO_Pin_11|GPIO_Pin_10|GPIO_Pin_9 ;
	portSet.GPIO_Mode=GPIO_Mode_IPU;
	GPIO_Init(GPIOD,&portSet);
// 	//PWM  继电器
...
// 	GPIO_Init(GPIOD,&portSet);
	//PWM  继电器
  portSet.GPIO_Pin=GPIO_Pin_6|GPIO_Pin_7|GPIO_Pin_8|GPIO_Pin_9 ;
	portSet.GPIO_Mode=GPIO_Mode_Out_PP;
	GPIO_Init(GPIOC,&portSet);
	portSet.GPIO_Pin=GPIO_Pin_11|GPIO_Pin_10 ;
	portSet.GPIO_Mode=GPIO_Mode_Out_PP;
	GPIO_Init(GPIOB,&portSet);
	portSet.GPIO_Pin=GPIO_Pin_15|GPIO_Pin_14 ;
	portSet.GPIO_Mode=GPIO_Mode_Out_PP;
	GPIO_Init(GPIOE,&portSet);
	//待定
...
// 	GPIO_SetBits(GPIOB,GPIO_Pin_5|GPIO_Pin_6|GPIO_Pin_7|GPIO_Pin_12|GPIO_Pin_13|GPIO_Pin_14);
	//EEPROM  LED
	GPIO_SetBits(GPIOB,GPIO_Pin_5|GPIO_Pin_6|GPIO_Pin_7|GPIO_Pin_14|GPIO_Pin_15);
...
void EXTI_Configuration(void)
{
	...
	EXTI_init.EXTI_Trigger=EXTI_Trigger_Falling;      //520自带配置
...
//	EXTI_init.EXTI_Trigger=EXTI_Trigger_Rising_Falling;
	...
	nvicInit.NVIC_IRQChannelPreemptionPriority=0;
	nvicInit.NVIC_IRQChannelSubPriority=1;
	nvicInit.NVIC_IRQChannel=EXTI15_10_IRQChannel;
	NVIC_Init(&nvicInit);
	#endif
}
void ADC_Configuration(void)
{
    ...
    ADC_Init(ADC1, &ADC_InitStructure);  // A0 A1 A2 A7 C0
...
//  ADC_RegularChannelConfig(ADC1, ADC_Channel_10,9, ADC_SampleTime_71Cycles5);
	  ADC_RegularChannelConfig(ADC1, ADC_Channel_14,9, ADC_SampleTime_71Cycles5);
...
{   // SYS1.CNt3到20000   大概3.12秒   156us一次中断
  OSIntNesting++;
...
void USART1_IRQHandler(void)
{
	...
	//         if(SYS1.CNt2<8) SYS1.Cata6[SYS1.CNt2]=data;		
						 if(SYS1.CNt2<10)  
							 SYS1.CNt2++ ;
						 else if((SYS1.Cata6[1]==0x16)&&(SYS1.CNt2<150))   
							 SYS1.CNt2++ ;
						 else if((SYS1.Cata6[0]==0x31)&&(SYS1.CNt2<30))    
							 SYS1.CNt2++ ;
							
						 SYS1.Cata6[SYS1.CNt2]=data ;
        }
        SYS1.CNt3=0;
        if((SYS1.CNt2==7)&&((SYS1.Cata6[0]==SYS.ID)||(SYS1.Cata6[0]==0x30))&&(SYS1.Cata6[1]!=0x16))
        {   
            DisLED(1); SYS1.CNt2=0;
            OSSemPost(SS1[0]);
        }
				//modfile
		if(SYS1.CNt2==147 && SYS1.Cata6[0]==SYS.ID && SYS1.Cata6[1]==0x16)
        {   
            DisLED(1); SYS1.CNt2=0;
            OSSemPost(SS1[0]);
        }
				//随机设置
		if((SYS1.CNt2==28)&&(SYS1.Cata6[0]==0x31)&&(SYS1.Cata6[1]==06))
        {   
            DisLED(1); SYS1.CNt2=0;
            OSSemPost(SS1[0]);
        }
...
void TaskStartDisplay(void *yydata)
{
	...
	GetCommID();
	//checkOff = GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_0)? 0:1;
 	checkOff = 1;
...
//         else SYS.PWM_GetPeriod4=0;
		    if (GPIOE->IDR & GPIO_Pin_9) SYS.PWM_GetPeriod1=0; 
        else {if(SYS.PWM_GetPeriod1<50) SYS.PWM_GetPeriod1++;}
        if (GPIOE->IDR & GPIO_Pin_8) SYS.PWM_GetPeriod2=0;
        else {if(SYS.PWM_GetPeriod2<50) SYS.PWM_GetPeriod2++;}
        if (GPIOE->IDR & GPIO_Pin_7) SYS.PWM_GetPeriod3=0;
        else {if(SYS.PWM_GetPeriod3<50) SYS.PWM_GetPeriod3++;}
        if (GPIOB->IDR & GPIO_Pin_1) SYS.PWM_GetPeriod4=0;
        else {if(SYS.PWM_GetPeriod4<50) SYS.PWM_GetPeriod4++;}
...
case 90: checkOff = dataBuff; CommIIC(&CHdata1.DataSet[Nop2],2,(Nop2<<1),0);break; 
 //power
...
//    u8 i=0,status,dataBuff[8];
	u8 *ptr ,i=0,temp ,temp1 ,status,dataBuff[8],dataBuff1[150];
	yydata=yydata;
//	OSTimeDly(10000);
    KeyTurnX(20);
    Reg4Data[130]=0x55;
	...
	{   
        OSSemPend(SS1[0],0,&status);
		    ...
		    if((SYS1.Cata6[0]==SYS.ID)&&(SYS1.Cata6[1]==0x16))  //modfile
				{																	
						OS_ENTER_CRITICAL();	
                        for(i=0;i<148;i++) 
						{
							dataBuff1[i]=SYS1.Cata6[i];
							SYS1.Cata6[i]=0x00; //0xff
						}  
						CCdataValue(&dataBuff1[0],146);
						OS_EXIT_CRITICAL();
						
						if((SYS1.Cata1==dataBuff1[146])&&(SYS1.Cata2==dataBuff1[147]))
						{
                 //dataBuff1开始处理成单个变量设置
							 		dataBuff[0]= SYS.ID;	
									dataBuff[1]= 06;
									dataBuff[2]= 00;
									ptr = &dataBuff1[2] ; 
...
// 									 }
				...
				#if(0)
				else if(SYS1.Cata6[0]==31)  //随机设置   32块pcb板
				{
					    OS_ENTER_CRITICAL();
              for(i=0;i<24;i++) 
							{
							...
							CCdataValue(&dataBuff1[0],22);
							OS_EXIT_CRITICAL();
							if((SYS1.Cata1==dataBuff1[22])&&(SYS1.Cata2==dataBuff1[23]))
							{   
								  dataBuff[0] = SYS.ID ;
								  dataBuff[1] = 06 ;
								  dataBuff[2] = 00 ;
								  dataBuff[3] = dataBuff1[3] ;
								  dataBuff[4] = dataBuff1[4] ;
								  dataBuff[5] = dataBuff1[5] ;
								  	
                  dataBuff[3]	++ ;							
								  if(SYS.ID%2)  //板号 1 3 5 7 9
									{											 											 
											 temp = dataBuff1[22-(SYS.ID+1)/2] ;
											 temp1 = dataBuff[3] ;
                  ...
                  {
										   //dataBuff[6]-dataBuff[21] 存储32块pcb 128个测温点
										   // dataBuff[21] 存储ID1 ID2   dataBuff[6]存储ID31 ID32
											 temp = dataBuff1[22-(SYS.ID+1)/2] ;
											 temp1 = dataBuff[3] ;
				...
				#endif
				else if(SYS1.Cata6[0]==0x31)  //随机设置   42块pcb板
				{
					    OS_ENTER_CRITICAL();
              for(i=0;i<29;i++) 
							{
							...
							CCdataValue(&dataBuff1[0],27);
							OS_EXIT_CRITICAL();
							if((SYS1.Cata1==dataBuff1[27])&&(SYS1.Cata2==dataBuff1[28]))
							{   
								  dataBuff[0] = SYS.ID ;
								  dataBuff[1] = 06 ;
								  dataBuff[2] = 00 ;
                  ...
                  {
										   //dataBuff[6]-dataBuff[26] 存储42块pcb 168个测温点
										   // dataBuff[26] 存储ID1 ID2   dataBuff[6]存储ID41 ID42
											 temp = dataBuff1[27-(SYS.ID+1)/2] ;
											 temp1 = dataBuff[3] ;
				...
				else
				{
							...
							if((SYS1.Cata1==dataBuff[6])&&(SYS1.Cata2==dataBuff[7]))
							{
									...
									else if(dataBuff[1]==4) 
									  {
										  Timcomm(Reg3Data,SYS.ID,4,dataBuff[3],dataBuff[5]);
									  }
									else if(dataBuff[1]==6) 
									{
											if(dataBuff[0]==0x30)        //广播
											{
												OS_ENTER_CRITICAL();
												for(i=0;i<4;i++) {SafeData(&dataBuff[0]);dataBuff[3]++;}
												OS_EXIT_CRITICAL();
											}
											else if(dataBuff[0]==SYS.ID) //单点设置
											{
												 SafeData(&dataBuff[0]);Timcomm(Reg4Data,SYS.ID,6,dataBuff[3]-1,1);
											}	
									} 

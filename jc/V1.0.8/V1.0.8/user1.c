...
//	dataBuff32s=dataBuff32s*SYS.DataCalSet[ET_Z]*330/(33*4095-33*dataBuff32s);
...
void GetTem1(void)
{
	...
	CHdata1.TemBakAD = CHdata1.TemBakAD*0.8 + last*0.2;
	last = CHdata1.TemBakAD;
...
void GetTem2(void)
{
	...
	CHdata2.TemBakAD = CHdata2.TemBakAD*0.8 + last*0.2;
	last = CHdata2.TemBakAD;
...
void GetTem3(void)
{
	...
	CHdata3.TemBakAD = CHdata3.TemBakAD*0.8 + last*0.2;
	last = CHdata3.TemBakAD;
...
void GetTem4(void)
{
	...
	CHdata4.TemBakAD = CHdata4.TemBakAD*0.8 + last*0.2;
	last = CHdata4.TemBakAD;
	
	dataBuff32s=(s32)buff1Num*SYS.DataCalSet[HT_Z4];	
	if(buffTem>dataBuff32s) buffTem-=dataBuff32s;
	else buffTem=0;
//	SYS.DataCalSet[HT_A4] = 0x03d4 ;  ////////////////////////
...
void CheckOutShort1(void)
{
	...
	OSTimeDly(2000); 
    GetCurrent1();
	if(CHdata1.DataGet[Current]>=30) 
	{
		OSTimeDly(5000);
	 	GetCurrent1();
	//	if(CHdata1.DataGet[Current]>=30) CHdata1.ErrorCode=TR_ST;
	}
	GetTem1(); 
...
void CheckOutShort2(void)
{
	...
	OSTimeDly(2000); 
    GetCurrent2();
	if(CHdata2.DataGet[Current]>=30) 
	{
		OSTimeDly(5000);
	 	GetCurrent2();
//		if(CHdata2.DataGet[Current]>=30) CHdata2.ErrorCode=TR_ST;
	}
	GetTem2();
...
void CheckOutShort3(void)
{
	...
	OSTimeDly(2000); 
    GetCurrent3();
	if(CHdata3.DataGet[Current]>=30) 
	{
		OSTimeDly(5000);
	 	GetCurrent3();
	//	if(CHdata3.DataGet[Current]>=30) CHdata3.ErrorCode=TR_ST;
	}
	GetTem3();
...
void CheckOutShort4(void)
{
	...
	OSTimeDly(2000); 
    GetCurrent4();
	if(CHdata4.DataGet[Current]>=30) 
	{
		OSTimeDly(5000);
	 	GetCurrent4();
	//	if(CHdata4.DataGet[Current]>=30) CHdata4.ErrorCode=TR_ST;//Check TRAC is short?
	}
	GetTem4();
...
void TaskStartPID1(void *yydata)
{
	...
	static s32 out,lastout,ek ,lastek,last;
    ...
    outBak=CHdata1.DataGet[Rate];
		...
		if(CHdata1.RunningMode==1)
		{
         ...
         if(CHdata1.AutoFlag==1) 
			...
			{
				if((CHdata1.TemPV*2)<(CHdata1.TemSV))
				{
					TurnData1.step=0;
					CHdata1.AutoFlag=0;
				}
...
//				else if(CHdata1.TemPV>(CHdata1.TemSV+1000)) MVresult1=0;
				...
				ek = CHdata1.TemSV-CHdata1.TemPV;
						
				out = MVresult1 + (ek-lastek)*CHdata1.DataSet[P]*6;  //6-8
				out = out*0.8 + lastout*0.2;									
				CHdata1.DataGet[Rate]=out/100;	
...
void TaskStartPID2(void *yydata)
{
    ...
    outBak=CHdata2.DataGet[Rate];
		...
		if(CHdata2.RunningMode==1)
		{
			...
			{
				if((CHdata2.TemPV*2)<(CHdata2.TemSV))
				{
					TurnData2.step=0;
					CHdata2.AutoFlag=0;
				}
...
//				else if(CHdata2.TemPV>(CHdata2.TemSV+1000)) MVresult1=0;
				...
				ek = CHdata2.TemSV-CHdata2.TemPV;
						
				out = MVresult1 + (ek-lastek)*CHdata2.DataSet[P]*6;  //6-8
				out = out*0.8 + lastout*0.2;									
				CHdata2.DataGet[Rate]=out/100;	
...
void TaskStartPID3(void *yydata)
{
    ...
    outBak=CHdata3.DataGet[Rate];	
		...
		if(CHdata3.RunningMode==1)
		{
			...
			{
				if((CHdata3.TemPV*2)<(CHdata3.TemSV))
				{
					TurnData3.step=0;
					CHdata3.AutoFlag=0;
				}
...
//				else if(CHdata3.TemPV>(CHdata3.TemSV+1000)) MVresult1=0;
				...
				ek = CHdata3.TemSV-CHdata3.TemPV;
						
				out = MVresult1 + (ek-lastek)*CHdata3.DataSet[P]*6;  //6-8
				out = out*0.8 + lastout*0.2;									
				CHdata3.DataGet[Rate]=out/100;	
		...
		GetCurrent3();
	//	if(!(GPIOB->IDR & GPIO_Pin_9)) check_TCRE_counter=23;
...
//		OSTimeDly(9995);
    ...
    outBak=CHdata4.DataGet[Rate];
		...
		if(CHdata4.RunningMode==1)
		{
			...
			{
				if((CHdata4.TemPV*2)<(CHdata4.TemSV))
				{
					TurnData4.step=0;
					CHdata4.AutoFlag=0;
				}
...
//				else if(CHdata4.TemPV>(CHdata4.TemSV+1000)) MVresult1=0;
				...
				ek = CHdata4.TemSV-CHdata4.TemPV;
						
				out = MVresult1 + (ek-lastek)*CHdata4.DataSet[P]*6;  //6-8
				out = out*0.8 + lastout*0.2;									
				CHdata4.DataGet[Rate]=out/100;	
...
void TIM2_IRQHandler(void)
{
	...
	{					
		...
		{	
			PWM_OUT(1,ON);	
			startDelay1 --;
			if(startDelay1 == 0) CHdata1.DataGet[Rate] = 0;					
		}		
		else
		{
			if(outfig == 1) PWM_OUT(1,ON);
			else outfig = 1;
		}
...
void TIM3_IRQHandler(void)
{
	...
	{					
		...
		{	
			PWM_OUT(2,ON);	
			startDelay2 --;
			if(startDelay2 == 0) CHdata2.DataGet[Rate] = 0;					
		}		
		else
		{
			if(outfig == 1) PWM_OUT(2,ON);
			else outfig = 1;
		}
...
void TIM4_IRQHandler(void)
{
	...
	{					
		...
		{	
			PWM_OUT(3,ON);	
			startDelay3 --;
			if(startDelay3 == 0) CHdata3.DataGet[Rate] = 0;					
		}		
		else
		{
			if(outfig == 1) PWM_OUT(3,ON);
			else outfig = 1;
		}
...
void TIM1_UP_IRQHandler(void)
{
	...
	{					
		...
		{	
			PWM_OUT(4,ON);	
			startDelay4 --;
			if(startDelay4 == 0) CHdata4.DataGet[Rate] = 0;					
		}		
		else
		{
			if(outfig == 1) PWM_OUT(4,ON);
			else outfig = 1;
		}
...
void EXTI0_IRQHandler(void)  //零点 PB0
{
    #if OS_CRITICAL_METHOD == 3
    OS_CPU_SR  cpu_sr;
    #endif

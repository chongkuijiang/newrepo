...
// 	}
		if(channel==1)
		{
			if(command) GPIOB->BRR=GPIO_Pin_11;
			else GPIOB->BSRR=GPIO_Pin_11;
		}
		else if(channel==2)
		{
			if(command) GPIOB->BRR=GPIO_Pin_10;
			else GPIOB->BSRR=GPIO_Pin_10;
		}
		else if(channel==3)
		{
			if(command) GPIOE->BRR=GPIO_Pin_15;
			else GPIOE->BSRR=GPIO_Pin_15;
		}
		else if(channel==4)
		{
			if(command) GPIOE->BRR=GPIO_Pin_14;
			else GPIOE->BSRR=GPIO_Pin_14;
		}
	OSTimeDly(500);
}
...
//     }
	    if(number==1)
			{
					if(command==ON) GPIO_ResetBits(GPIOB,GPIO_Pin_15);
			...
			{
					if(command==ON) GPIO_ResetBits(GPIOD,GPIO_Pin_8);
...
//     } 
	    if(number==1)
			{
					if ((GPIOB->ODR & GPIO_Pin_15) != (u32)Bit_RESET) GPIOB->BRR = GPIO_Pin_15;
			...
			{
					if ((GPIOD->ODR & GPIO_Pin_8) != (u32)Bit_RESET) GPIOD->BRR = GPIO_Pin_8 ;
					else GPIOD->BSRR = GPIO_Pin_8;
			} 
}
...
void LoadData(void)
{
	...
	{
		if(CHdata1.DataSet[Nop0]==0x5a2)
		{
		 	CommIIC(CHdata1.DataSet,50,0,1);
			Delay(100);
            CommIIC(CHdata2.DataSet,50,64,1);
			Delay(100);
            CommIIC(CHdata3.DataSet,50,128,1);
			Delay(100);
            CommIIC(CHdata4.DataSet,50,192,1);
			Delay(100);
		...
		else 
		{
            ...
            SetInitData();
			CommIIC(CHdata1.DataSet,50,0,0);
			Delay(300);
            CommIIC(CHdata2.DataSet,50,64,0);
			Delay(300);
            CommIIC(CHdata3.DataSet,50,128,0);
			Delay(300);
            CommIIC(CHdata4.DataSet,50,192,0);
			Delay(300);
            CommIIC(SYS.DataCalSet,20,256,0);
		}
	}
//	checkOff = CHdata1.DataSet[Nop2];
	checkOff = 1;
...
void GetCommID(void)
{
  //  unsigned char dataBuff[5]={251,252,253,254,255},i=0;
	unsigned short dataBuff[5]={251,252,253,254,255},i=0;
...
// 	if(!(dataBuff[0]&0x20)) SYS.ID+=1;
	...
	if(!(dataBuff[0]&0x0800)) SYS.ID+=16;
	if(!(dataBuff[0]&0x1000)) SYS.ID+=8;
	if(!(dataBuff[0]&0x2000)) SYS.ID+=4;
	if(!(dataBuff[0]&0x4000)) SYS.ID+=2;
	if(!(dataBuff[0]&0x8000)) SYS.ID+=1;
}
...
//         SYS.AD_AddValue[7]+=SYS.AD_Value[7];
	      SYS.AD_AddValue[4]+=SYS.AD_Value[7];
        SYS.AD_AddValue[5]+=SYS.AD_Value[6];
        SYS.AD_AddValue[6]+=SYS.AD_Value[5];
        SYS.AD_AddValue[7]+=SYS.AD_Value[4];

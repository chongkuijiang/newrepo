...
#define EN_NVIC_2     1
#define PreemptionPriority_2  0
#define SubPriority_2  1
#define ReMap_2    0  
//----------------------------------------------------------------------------------------------------------------------
// <e0> 使能串口3    
...
void USART1_IRQHandler(void)                	//串口1中断服务程序
{
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  
	{
		...
		Rx[0].Rxbuf[Rx[0].Rxp] = USART_ReceiveData(USART1);
		if(Rx[0].Rxp < 19) Rx[0].Rxp++;
		Rx[0].RxDelay = 3; //0.5ms
    } 
...
void USART2_IRQHandler(void)                	//串口2中断服务程序
{
	if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)  
	{
		...
		Rx[1].Rxbuf[Rx[1].Rxp] = USART_ReceiveData(USART2);
		if(Rx[1].Rxp < 19) Rx[1].Rxp++;
		Rx[1].RxDelay = 3; //0.5ms
     } 
...
void USART3_IRQHandler(void)                	//串口3中断服务程序
{
	...
	if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)  
	{
	    ...
	    Rx[2].Rxbuf[Rx[2].Rxp] = USART_ReceiveData(USART3);
		if(Rx[2].Rxp < 19) Rx[2].Rxp++;
		Rx[2].RxDelay = 3; //0.5ms
     } 

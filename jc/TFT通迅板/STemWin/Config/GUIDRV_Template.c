...
#include "LCD_ConfDefaults.h"
  ...
  void LCD_DrawPoint(unsigned short x,unsigned short y,unsigned short Pixel);
  unsigned short LCD_ReadPoint(unsigned short x,unsigned short y);
  void LCD_Color_Fill(int x0, int y0, int x1, int y1,unsigned short color);
  void DrawBitLine(int x, int y, unsigned short const * p, int xsize);
/*********************************************************************
*
*       Defines
*
...
static void _FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1) {
  ...
  if (GUI_pContext->DrawMode & LCD_DRAWMODE_XOR) {
    LCD_Color_Fill(x0, y0, x1, y1,PixelIndex);
  } else {
    for (; y0 <= y1; y0++) {
      for (x = x0; x <= x1; x++) {

...
void GPIO_Configuration(void)
{
   ...
   portSet.GPIO_Pin=GPIO_Pin_6|GPIO_Pin_7|GPIO_Pin_8|GPIO_Pin_9;
	...
	GPIO_Init(GPIOC,&portSet);
	//4路继电器
	portSet.GPIO_Pin = GPIO_Pin_10 | GPIO_Pin_11 ;
   ...
   // 3路LED
	...
	GPIO_Init(GPIOC,&portSet); 
	portSet.GPIO_Pin = GPIO_Pin_6 ;
	portSet.GPIO_Mode = GPIO_Mode_IN_FLOATING;
	GPIO_Init(GPIOE,&portSet);
	// 识别ID	
	portSet.GPIO_Pin = GPIO_Pin_9 | GPIO_Pin_10 | GPIO_Pin_11 | 
   ...
   portSet.GPIO_Mode = GPIO_Mode_IN_FLOATING;
   GPIO_Init(GPIOB,&portSet);
	portSet.GPIO_Pin = GPIO_Pin_7 | GPIO_Pin_8 | GPIO_Pin_9;
	portSet.GPIO_Mode = GPIO_Mode_IN_FLOATING;
   GPIO_Init(GPIOE,&portSet);
	//4路交流故障
...
void ADC_Configuration(void)
{
    ...
    ADC_RegularChannelConfig(ADC1, ADC_Channel_14,9, ADC_SampleTime_71Cycles5);//ntc
    ADC_Cmd(ADC1, ENABLE);
...
void USART1_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)
  	{
	//	Rbuf[p++] = USART_ReceiveData(USART1);
...
void TaskStartDisplay(void *yydata)
{
	...
	while(1)
	{
        ...
        if (!(GPIOE->IDR & GPIO_Pin_9)) {
		      if(SYS.PWM_GetPeriod1<50) SYS.PWM_GetPeriod1++;
		  }
        else SYS.PWM_GetPeriod1=0;
        if (!(GPIOE->IDR & GPIO_Pin_8)) {
		     if(SYS.PWM_GetPeriod2<50) SYS.PWM_GetPeriod2++;
		  }
        else SYS.PWM_GetPeriod2=0;
        if (!(GPIOE->IDR & GPIO_Pin_7)) {
		     if(SYS.PWM_GetPeriod3<50) SYS.PWM_GetPeriod3++;
		  }
        else SYS.PWM_GetPeriod3=0;
        if (!(GPIOB->IDR & GPIO_Pin_1)) {
		     if(SYS.PWM_GetPeriod4<50) SYS.PWM_GetPeriod4++;
		  }

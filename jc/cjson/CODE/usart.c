...
//---------------------------------------------------------------------------------------- 
  ...
  uint8_t Rxbuf1[2048],rxtim1;
  uint8_t Txbuf1[1024],txlen1;
  uint16_t pr1,pt1;
...
void sendn(USART_TypeDef* mUsart,uint8_t* txbuf,uint8_t* txlen,char* buf){   
	...
	*txlen = i;
	if(SEND_NODE == 1){
		...
		for(i=0;i<7;i++){
		   USART_SendData(mUsart, txbuf[i]);
		   while (USART_GetFlagStatus(mUsart, USART_FLAG_TXE) == RESET);	
		}	
	}
}
//-----------------------------------------------------------------
